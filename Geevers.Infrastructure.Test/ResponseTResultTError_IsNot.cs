namespace Geevers.Infrastructure.Test
{
	...
	[TestClass]
	public class ResponseTResultTError_IsNot
	{
		[TestMethod]
		public void IfResponseWithResultStatus_DoesNot_MatchCondition_IsNot_ReturnsTrueAndOutVarsAreSet()
		{
			// arrange
			...
			var responses = statii
				.Select(cue =>
				{
					var a = response.IsNot(cue, out var result, out var status, out var error);

					return (InStatus: cue, IsResult: a, OutResult: result, OutStatus: cue, OutError: error);
				});

			// assert
			Assert.IsTrue(responses.All(p => true == p.IsResult));
		...
		[TestMethod]
		public void IfResponseWithErrorStatus_DoesNot_MatchCondition_IsNot_ReturnsTrueAndOutVarsAreSet()
		{
			// arrange
		...
		[TestMethod]
		public void IfResponseWithResultStatus_Does_MatchCondition_IsNot_ReturnsFalseAndOutVarsAreSet()
		{
			// arrange
			Response<int, Error> response = (HttpStatusCode.Created, 42);

			// act
			var a = response.IsNot(HttpStatusCode.Created, out var result, out var status, out var error);
		...
		[TestMethod]
		public void IfResponseWithErrorStatus_Does_MatchCondition_IsNot_ReturnsFalseAndOutVarsAreSet()
		{
			// arrange
			Response<int, Error> response = (HttpStatusCode.Conflict, Error.UniverseExploded);

			// act
			var a = response.IsNot(HttpStatusCode.Conflict, out var result, out var status, out var error);
